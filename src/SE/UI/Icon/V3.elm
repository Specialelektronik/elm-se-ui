module SE.UI.Icon.V3 exposing
    ( Icon, toHtml, toDataUri, withContainerSize, withSize
    , Transform(..), withTransform
    , angleDown, ban, bargain, box, campaign, cart, category, checkCircle, clock, envelope, facebook, file, history, instagram, lightbulb, linkedin, phone, playCircle, search, star, table, grid, thList, trash, user, youtube
    , tick, circle
    , sync
    )

{-| Version 3 of Icons, mimics Bulma Icon element set, now with icons from Iconoir.
...
Version 2 and 3 uses withStar-pattern to allow transformation and support both icon containers and all icons as DataUris (to use as Css backgrounds)
...
@docs angleDown, ban, bargain, box, campaign, cart, category, checkCircle, clock, envelope, facebook, file, history, instagram, lightbulb, linkedin, phone, playCircle, search, filter, star, table, grid, thList, trash, user, youtube
...
type Shape
    ...
    | Circle { centerX : Float, centerY : Float, radius : Float }
    | Line { x1 : Float, y1 : Float, x2 : Float, y2 : Float }


initInternals : ( Int, Int, List String ) -> Internals
initInternals ( width, height, paths ) =
    Internals Control.Regular Control.Regular [] ( width, height ) (List.map Path paths)


initInternalsV3 : ( Int, Int ) -> List Shape -> Internals
initInternalsV3 =
...
angleDown : Icon
angleDown =
    Icon (initInternalsV3 ( 24, 24 ) [ Path "M6 9L12 15L18 9" ])
...
ban =
    Icon
        (initInternalsV3 ( 24, 24 )
            [ Circle { centerX = 12, centerY = 12, radius = 10 }
...
sync : Icon
sync =
    Icon (initInternals ( 512, 512, [ "M370.72 133.28C339.458 104.008 298.888 87.962 255.848 88c-77.458.068-144.328 53.178-162.791 126.85-1.344 5.363-6.122 9.15-11.651 9.15H24.103c-7.498 0-13.194-6.807-11.807-14.176C33.933 94.924 134.813 8 256 8c66.448 0 126.791 26.136 171.315 68.685L463.03 40.97C478.149 25.851 504 36.559 504 57.941V192c0 13.255-10.745 24-24 24H345.941c-21.382 0-32.09-25.851-16.971-40.971l41.75-41.749zM32 296h134.059c21.382 0 32.09 25.851 16.971 40.971l-41.75 41.75c31.262 29.273 71.835 45.319 114.876 45.28 77.418-.07 144.315-53.144 162.787-126.849 1.344-5.363 6.122-9.15 11.651-9.15h57.304c7.498 0 13.194 6.807 11.807 14.176C478.067 417.076 377.187 504 256 504c-66.448 0-126.791-26.136-171.315-68.685L48.97 471.03C33.851 486.149 8 475.441 8 454.059V320c0-13.255 10.745-24 24-24z" ] ))
...
toHtml (Icon internals) =
    ...
    styled Html.span
        ...
        [ Svg.svg [ attribute "aria-hidden" "true", Attributes.strokeWidth "1.5", Attributes.fill "none", Attributes.stroke "currentColor", Attributes.strokeLinecap "round", Attributes.strokeLinejoin "round", Attributes.width s, Attributes.height s, viewBoxtoHtml internals.viewBox ] (List.map shapeToSvg internals.shapes) ]


shapeToSvg : Shape -> Svg msg
shapeToSvg shape =
    case shape of
        Path str ->
            Svg.path [ Attributes.d str ] []

        Rect { x, y, width, height } ->
            Svg.rect [ Attributes.x (String.fromFloat x), Attributes.y (String.fromFloat y), Attributes.width (String.fromFloat width), Attributes.height (String.fromFloat height) ] []

        Circle { centerX, centerY, radius } ->
            Svg.circle
                [ Attributes.cx (String.fromFloat centerX)
                , Attributes.cy (String.fromFloat centerY)
...
toDataUri (Icon internals) =
    let
        ( w, h ) =
            internals.viewBox
    in
    "data:image/svg+xml,%3Csvg viewBox='0 0 " ++ String.fromInt w ++ " " ++ String.fromInt h ++ "' fill='currentColor' xmlns='http://www.w3.org/2000/svg'%3E" ++ (List.map shapetoDataUri internals.shapes |> String.join "") ++ "%3C/svg%3E"


shapetoDataUri : Shape -> String
shapetoDataUri shape =
    case shape of
        Path str ->
            "%3cpath d='" ++ str ++ "'/%3e"

module SE.UI.Navbar.V2 exposing
    ( Config, Brand(..), Model, defaultModel, Msg, update
    , view
    , Item(..), Link, MegaItem
    )

{-| A highly customized version of Bulmas navbar component. V2 has an added option for mega items without content, just a link.
...
viewMegaItem config model index item =
    case item.content of
        Just content ->
            viewMegaItemWithContent config model index { label = item.label, href = item.href, content = content }

        Nothing ->
            viewMegaItemLink config model index { label = item.label, href = item.href }


viewMegaItemWithContent : Config msg -> Model -> Int -> { label : String, href : Html.Attribute msg, content : Html msg } -> Html msg
viewMegaItemWithContent config model index item =
    let
        id =
...
viewMegaItemLink : Config msg -> Model -> Int -> { label : String, href : Html.Attribute msg } -> Html msg
viewMegaItemLink config model index item =
    Html.li
        []
        [ styled Html.a
            megaItemStyles
